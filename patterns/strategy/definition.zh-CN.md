...
- **ConcreteStrategy：**Fast Scan、Deep Review 子 Skill 及对应 Python 对象。
  两个内置实现在同一操作和请求/结果契约后拥有不同规则集。注入的替代实现可以使用
  其他算法，只需满足该公共契约。
...
顺序、发现结构与唯一身份，以及严格类型的派生汇总。JSON 成员顺序不影响语义；边界
会按请求文件、行号和规则 ID 规范化输出字段与发现顺序。Context 不检查注入过程，
也不要求它使用内置规则。
...
选择与委托分开验证，并覆盖同一 Context 中的替换、与公共契约测试分离的内置规则
归属、规范输出、唯一发现身份、严格汇总整数类型、无语义成员顺序、重复 JSON 成员和
孤立代理项拒绝、错误策略结果拒绝、精确输出、稳定错误、输入不变性，以及无网络和
模型依赖的标准库执行。
...
规则行为，逐字精简兼容 API，严格公共结果字段，规范顺序和唯一发现身份，错误注入
结果，以及 schema、类型、边界、UTF-8、代理项、重复成员 JSON 和路径校验、
确定性重跑与输入不变性。仓库根 harness 会复制并独立运行本样例。

## 局限（Limitations）

一个构造场景不能证明生态普遍性、比较收益、生产审查准确率或跨 Host 行为等价。扩展
请求、规则集、阈值、严重性策略或结果都需要版本化契约变更和新一致性夹具。策略可互换
不表示成本或发现结果相同，而是要求它们可通过声明的接口相互替换。精简兼容 API 与
富 CLI 是两个显式分离的公共契约，不能被当作彼此可互换。
