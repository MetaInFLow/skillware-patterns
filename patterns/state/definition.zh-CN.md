...
- **Context：**根供应商接入工作流 Skill 和确定性 `VendorWorkflow`。它提供统一
...
才更新内存 State。新建 Context 可在重启后恢复同一持久状态。
...
重载、重启恢复、损坏校验和写入前拒绝。聚焦测试还证明精确输出、确定性重跑、输入
不变，以及写入失败时内存与磁盘一致。
...
本记录未评估生态对应，没有引用公共仓库、固定修订或源码路径，也不提出 candidate
或 confirmed 声明。本地构造样例不得被表述成外部证据。详情见
...
恢复、逐状态处理职责、非法转换不写入、原子替换失败、损坏状态、供应商身份不匹配、
精确夹具输出与错误、确定性重跑，以及无需网络的标准库独立执行。
