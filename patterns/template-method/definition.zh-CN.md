...
显式分派、继承的 `run` 不参与分派、直接重写拒绝、局部快照、失败终止、
严格输入/钩子/结果校验、边界复制、稳定错误、精确夹具与确定性重跑。

## 反向证据（Counter-Evidence）

Python 语言约束和测试不能证明 Agent Runtime 会以相同方式解释
Behavioral Source，也不能证明 Agent Host 会激活预期制品。领域输出只是
夹具内容，不是经证实的行业专业意见。钩子是协作式、受信任的扩展代码，
不是被沙箱或安全隔离的代码。深拷贝与局部快照只展示通过已声明钩子参数进行普通修改时的行为，
不防护闭包捕获的引用、模块全局修改、monkeypatch、内省、独立嵌套调用或其他任意进程内 Python 行为。
...
一次、共享契约的两个 ConcreteClass、显式非绑定分派、继承的 BypassMixin `run` 不参与分派、
静态钩子签名校验、钩子参数副本的普通修改、未知阶段返回字段拒绝、直接重写准入、
有界替换、钩子失败、畸形结果、确定性夹具、输入副本行为、输出复制、重复 JSON 成员、
无效 UTF-8、孤立代理项、循环、深度和类型。
