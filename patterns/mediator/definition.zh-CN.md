...
公开 API 在构造 ConcreteMediator 前完成全部状态校验和复制。ConcreteMediator 随后在不
绑定任何对象的前提下预检整个注入集合，包括类型、非空字符串 ID、精确且不重复的身份以及
未绑定状态；拒绝时所有原本未绑定的 Colleague 仍可复用。全部准入通过后，才按构建、安全、
文档、审批顺序绑定到自身地址。每个 Colleague 只收到自己的状态并被寻址一次。Colleague
