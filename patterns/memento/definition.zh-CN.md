...
非符号链接、UTF-8、JSON 结构与边界，然后保存精确字节和元数据。它用同目录
临时文件原子替换为 `n + 1` 版本，再重读校验。成功时 Caretaker 只使检查点过期，
不恢复；捕获后任何失败都通过 Originator 原子恢复。只有恢复成功才销毁检查点。
...
测试覆盖字面回滚 API、成功迁移不恢复、字节和权限恢复、写入与写后校验失败、恢复
失败报告与重试、过期与跨目标检查点、缺失/损坏/非 UTF-8 输入、重复字段、严格类型、
...
安全边界；反射、monkeypatch、任意内存访问或直接文件修改都可绕过它们。生产使用需要显式
