...
回滚编排还会把所有者、生命周期、对象标识、目标或校验和准入失败纳入恢复错误边界，使原始迁移错误与
恢复/准入错误同时保留。若该失败发生在新版文件已提交之后，由于不能安全应用受损检查点，完整新版文件可能保留。
...
多进程隔离或对恶意进程内代码的安全性。恢复操作、准入或完整性失败可能保留完整的新版文件；`MigrationRollbackError`
会保留原始迁移错误与恢复错误，并明确说明
...
损坏检查点的提交与丢弃拒绝、
写后检查点损坏、回滚准入包装、双错误保留以及无法恢复时的完整新版文件结果、
