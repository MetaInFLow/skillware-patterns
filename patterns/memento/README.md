...
successful run atomically increments `version` and disposes its checkpoint. Any
migration or post-write validation failure invokes the owned restore path; the
original bytes and portable permission bits are atomically reinstated before
the original error is re-raised. A failed restore is reported explicitly and
never mislabeled as recovery.
