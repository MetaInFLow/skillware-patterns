...
gstack 案例为生成式和系统专属 Host 表面提供**强对应（strong
correspondence）**证据。
...
其生成器与系统专属 Host 配置把规范 Skill 源转换为目标发现方式、frontmatter、
路径和工具名约定。规范 Skill 语义是 Adaptee，每个生成或系统专属绑定是
Adapter，Host 专属 Skill 契约是 Target，任务级调用是 Client。固定链接与运行
等价限制见 [`correspondence.md`](correspondence.md)。该生态对应主张与本地构造
样例相互独立。
...
一个构造性场景和一个强生态对应不能证明模式使用频率、比较质量或跨 Host 运行
