...
## 典范来源（Canonical Source）

装饰模式是 Gamma、Helm、Johnson 和 Vlissides 在 1994 年
*Design Patterns: Elements of Reusable Object-Oriented Software* 中定义的经典
结构型模式，历史上属于 Gang of Four 目录。本转移保留 Component、
ConcreteComponent、Decorator 和 ConcreteDecorator 的源角色，不声称新模式。
...
发现，Citation Check 追加缺失引用标记发现，可选 Compliance Check 追加合规标记发现，
但不属于原样计划的默认组合。它们都仍然是有效 Component，可以包装基础组件或相互包装。
...
完整内层结果，然后只在精确 `(type, message)` 身份不存在时追加自己的发现。
Component 结果中的发现身份必须唯一，因此重复的同一包装器具有幂等性。
...
装饰后的 Component，也可以独立增加新的有界装饰器。结果契约不对发现数量设定
有限上限，使其他合法装饰器继续追加发现时，组件仍然可替换。
...
| ConcreteDecorator | 隐私、引用和可选合规检查 Skill Artifact | `sample/child-skills/privacy-check/SKILL.md`, `sample/child-skills/citation-check/SKILL.md`, `sample/child-skills/compliance-check/SKILL.md` |

Behavioral Source 定义并影响根与子 Skill Artifact，这些 Skill Artifact 组成构造性
Skillware Unit；Skill 文件本身是 Artifact，不是 Behavioral Source。Python 预言机提供相同
参与者关系的可调用对象，但不声称 Agent Runtime 会解释 Behavioral Source。
...
加载 manifest 的典范标识，反转包装顺序，增加可选合规检查，注入替代基础组件，
验证超过 100 个包装器和发现，强制唯一身份与幂等性，校验 key 类型、边界、循环与深度，
原样传播异常，以及请求/结果别名隔离。
...
Caveman 是固定修订 `25d22f864ad68cc447a4cb93aefde918aa4aec9f` 上的
**候选对应**。其激活 hook 读取典范 Skill Artifact，围绕会话启动增加行为，并继续使用
hook 进程/stdout Host 交互面。这两个路径未证明完整 GoF Component 契约、可替换性、
失败等价性或 Agent Runtime 行为。见 [`correspondence.md`](correspondence.md) 和
...
可选合规组合、超过 100 个包装器与发现、唯一身份与幂等性、匹配与不匹配行为、
原样失败传播、确定性输出，以及严格请求、结果、发现、key、字符串边界、UTF-8、代理项、
重复 JSON 成员、循环、parser 递归和深度校验。根仓库 harness 会独立复制并运行样例。

## 局限（Limitations）

样例中的邮箱正则、`[missing]` 和 `[noncompliant]` 字面标记不是生产级隐私、引用、合规或
法律审查。一个构造性样例不能证明生态普及度、跨 Host 等价性、模型解释、生产质量或比较收益。
Python 边界复制只是对所有权策略的建模，不证明 Agent Runtime 会强制执行该策略。
装饰器之间发现冲突的解决策略不在本契约范围内。
