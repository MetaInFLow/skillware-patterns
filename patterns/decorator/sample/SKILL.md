...
Decorator role. Privacy Check and Citation Check are ConcreteDecorator child
Skills.
...
3. Invoke the resulting Component once. At every wrapper boundary, pass an
   ...
   that wrapper's bounded finding when its condition matches.
4. Preserve exactly the `summary` and `findings` top-level fields. Preserve the
   ...
   finding before an outer wrapper's finding.
...
The default composition is Base Contract Review, then Privacy Check, then
Citation Check. In Python notation it is
...
基础审查逻辑；先追加内层结果，再追加外层结果。每个边界都要复制并
...
The two lexical checks are bounded demonstrations, not legal, privacy, or
citation assurance. A hook is not automatically a Decorator. It must wrap a
