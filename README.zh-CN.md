...
[论文](https://arxiv.org/abs/2607.18970) · [发布版本 `v0.1-paper-v1`](https://github.com/MetaInFLow/skillware-patterns/releases/tag/v0.1-paper-v1) · [浏览模式](patterns/)
...
| [`MetaInFlow/skillware-patterns`](https://github.com/MetaInFLow/skillware-patterns)（本仓库） | 发布迁移协议、有来源的目录、参与者映射、可运行示例、误用案例和聚焦测试。 | 这份自包含的可执行补充提供检查与运行其主张所需的本地证据；构造性输出不能验证本体，也不能证明生态普遍性。 |
...
每个 GoF 记录都给出至少一个实际公开 Skill 项目、固定提交、精确路径和受控状态。星数是 **2026-07-23** 的发现性快照，不是证据强度或质量评级；完整链接、路径和局限见 [`docs/upstream-skill-evidence.md`](docs/upstream-skill-evidence.md)，每个模式 README 也保留对应入口。

代表性入口包括 [obra/superpowers](https://github.com/obra/superpowers) 的 `using-superpowers`， [garrytan/gstack](https://github.com/garrytan/gstack) 的 Host 绑定，[calesthio/OpenMontage](https://github.com/calesthio/OpenMontage) 的 pipeline/checkpoint，[affaan-m/ECC](https://github.com/affaan-m/ECC) 的 lifecycle hooks，以及 [nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) 的路由 Skill。上游项目没有被强行解释为使用 GoF 术语；本地示例与上游观察是分开的证据主张。
...
完整的准入契约、验证要求及贡献许可边界见[贡献指南](CONTRIBUTING.md)。参与本项目须遵守 [Contributor Covenant 行为准则](CODE_OF_CONDUCT.md)。[公开仓库](https://github.com/MetaInFLow/skillware-patterns)通过论文发布版本 [`v0.1-paper-v1`](https://github.com/MetaInFLow/skillware-patterns/tree/v0.1-paper-v1)与[公开 arXiv 论文](https://arxiv.org/abs/2607.18970)绑定；其[验证工作流](https://github.com/MetaInFLow/skillware-patterns/actions/workflows/validate.yml)已配置为在每次 push 和 pull request 时运行。

## 安全

仓库只运行本地确定性示例，不需要密钥、网络或第三方服务凭据。不要把凭据写入 fixture 或对应记录；安全问题请通过[私密 GitHub 安全报告页面](https://github.com/MetaInFLow/skillware-patterns/security)提交，不要直接开公开 issue。

## 社区与项目链接

- [Issues](https://github.com/MetaInFLow/skillware-patterns/issues)：报告可复现缺陷或文档问题。
- [Pull requests](https://github.com/MetaInFLow/skillware-patterns/pulls)：提交审阅变更。
- [行为准则](CODE_OF_CONDUCT.md)：参与规范。
- [论文](https://arxiv.org/abs/2607.18970)：本体与研究主张。
